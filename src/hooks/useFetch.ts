...
const useFetch = <T>(fetchFunction: () => Promise<T>, autoFetch = true) => {  // useFetch hook takes a fetch function and an optional autoFetch boolean 
    ...
    const reset = () => { // reset state to initial values
        setData(null);
        setError(null);
        setLoading(false);
    }
